...
class DensityAnalyzer:
    ...
    def plot_densities(self, results, t_values, output_dir="plots/density"):
        """Genera gráficos de densidad total vs tiempo para distintos valores de p"""
        os.makedirs(output_dir, exist_ok=True)

        for t_value in t_values:
            # Filtrar resultados para este valor de t
            filtered_results = {p: data for (t, p), data in results.items() if t == t_value}
            ...
            plt.tight_layout()
            plt.savefig(f"{output_dir}/density_vs_time_t{t_value}.png", bbox_inches='tight', dpi=300)
            plt.close()
...
def main():
    ...
    analyzer.plot_densities(results, t_values)
