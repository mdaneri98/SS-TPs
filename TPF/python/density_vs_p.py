...
class DensityAnalyzer:
    ...
    def plot_average_density_vs_p(self, results, t_value, output_dir="plots/average_density"):
        """Genera un gráfico de densidad media vs p para un valor fijo de t"""
        os.makedirs(output_dir, exist_ok=True)

        # Filtrar resultados para el valor específico de t
        densities = []
        p_values = []

        for (current_t, p_value), data in results.items():
            if current_t == t_value:
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/average_density_t{t_value}.png", bbox_inches='tight', dpi=300)
        plt.close()
...
def main():
    ...
    t_values = [10]  # Valores de tiempo de redecisión
    ...
    # Generar gráficos de densidad media vs p para cada t
    for t_value in t_values:
        print(f"Generando gráfico para t = {t_value}...")
        analyzer.plot_average_density_vs_p(results, t_value)
