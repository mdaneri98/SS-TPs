...
class BpAnalyzer:
    ...
    def analyze_bp_variation(self, ap_value):
        """
        Analiza todas las simulaciones para un valor de ap dado
        """
        results = []
        heuristic_path = self.base_path / "heuristic_analysis"
        
        for bp in self.bp_values:
            dir_name = f"ap_{ap_value:.2f}_bp_{bp:.2f}"
            ...
            logging.info(f"Analizando Bp = {bp}")
            ...
            if simulation_distances:
                ...
                logging.info(f"Bp={bp}: media={results[-1]['mean_distance']:.2f}, "
...
if __name__ == "__main__":
    try:
        ...
        analyzer.bp_values = bp_values
        
        print(f"\nValores de Ap disponibles: {ap_values}")
        print(f"Valores de Bp disponibles: {bp_values}")
        
        # Usar el primer valor de ap encontrado
        ap_value = ap_values[0]
        print(f"\nAnalizando con Ap = {ap_value}")
        
        results = analyzer.analyze_bp_variation(ap_value)
        
        if not results.empty:
            analyzer.plot_bp_analysis(results)
            print("\nEstadísticas de distancia media vs Bp:")
